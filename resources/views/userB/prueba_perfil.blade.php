...
<x-configuracion-cuenta-card>
    ...
    <?php
        ...
        $usuariosA = App\Models\UserA::all();
        $usuariosB = App\Models\UserB::all();

        echo '<br>Usuarios A<br>';
        foreach($usuariosA as $usuario){
            echo $usuario->user->name.': ';
            echo $usuario->user->archivos()->where('archivo_type', 'img_perf')->get('ruta_archivo')[0]->ruta_archivo;    
        }


        echo '<br><br>Usuarios B<br>';
        foreach($usuariosB as $usuario){
            echo $usuario->user->name.': ';
            echo $usuario->user->archivos()->where('archivo_type', 'img_perf')->get('ruta_archivo')[0]->ruta_archivo;    
        }
    ...
    <div class="rounded-full">
        <img class="rounded-full" src="{{asset('storage/'.$ruta->ruta_archivo)}}" alt="" width="100" height="100">
    </div>
